...
RGBTRIPLE setBlurPixel(int startI, int endI, int startJ, int endJ, int totalPixelSum, int height,
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            if (i == 0)
            {
                if (j == 0)
                {
                    tempMatrix[i][j] = setBlurPixel(i, i+1, j, j+1, 4, height, width, image);
                }
                else if (j == width - 1)
                {
                    tempMatrix[i][j] = setBlurPixel(i, i+1, j-1, j, 4, height, width, image);
                }
                else
                {
                    tempMatrix[i][j] = setBlurPixel(i, i+1, j-1, j+1, 6, height, width, image);
                }
            }
            else if (i == height - 1)
            {
                if (j == 0)
                {
                    tempMatrix[i][j] = setBlurPixel(i-1, i, j, j+1, 4, height, width, image);
                }
                else if (j == width - 1)
                {
                    tempMatrix[i][j] = setBlurPixel(i-1, i, j-1, j, 4, height, width, image);
                }
                else
                {
                    tempMatrix[i][j] = setBlurPixel(i-1, i, j-1, j+1, 6, height, width, image);
                }
            }
            else
            {
                if (j == 0)
                {
                    tempMatrix[i][j] = setBlurPixel(i-1, i+1, j, j+1, 6, height, width, image);
                }
                else if (j == width - 1)
                {
                    tempMatrix[i][j] = setBlurPixel(i-1, i+1, j-1, j, 6, height, width, image);
                }
                else
                {
                    tempMatrix[i][j] = setBlurPixel(i-1, i+1, j-1, j+1, 9, height, width, image);
                }
            }
        }
    }
